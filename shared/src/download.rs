...
fn download(file_url: &str, cfg: &config::Config) -> Result<files::File, Box<dyn std::error::Error>> {
...
mod tests {
    ...
    fn test_download_csv() {
        ...
        let file =  files::File::new(file_url, &cfg);        let expected_file_name = "annual-enterprise-survey-2023-financial-year-provisional-size-bands.csv";
        ...
        match result {
            Ok(r) => {
                println!(":::: Finished downloading {}, duration: {}, file_size: {}",
                    &r.file_name,
                    &r.download_duration,
                    &r.file_size,
                    );
    ...
    fn test_download_vlc() {
        ...
        let file =  files::File::new(file_url, &cfg);        let expected_file_name = "vlc-3.0.21-arm64.dmg";
